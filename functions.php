...
function show_posts($userid){
    $posts = array();
 	$db = new bdd();
    $query = $db->getBdd()->prepare('SELECT tweet.content, tweet.created_date, user.username, follow.id_user, tweet.id, follow.id_user_follow, tweet.id_user FROM tweet LEFT JOIN user ON tweet.id_user = user.id LEFT JOIN follow ON  follow.id_user_follow = user.id
